...
2. *Calculate and report the __mean__ and __median__ total number of steps taken per day*
  ...
  The **mean** is *`r mean(agg_by_day$steps, na.rm=TRUE)`* and the **median** is *`r median(agg_by_day$steps, na.rm=TRUE)`*.
...
1. *Make a time series plot (i.e. `type = "l"`) of the 5-minute interval (x-axis) and the average number of steps taken, averaged across all days (y-axis)*
  ...
  plot(agg_by_interval$interval, agg_by_interval$steps, type='l', col="blue", main="Mean Number of Steps by 5 minute Interval", ylab="# of steps", xlab="5 Minute Interval")
  ```
2. *Which 5-minute interval, on average across all the days in the dataset, contains the maximum number of steps?*
  ...
  The 5-minute interval `r agg_by_interval[max(agg_by_interval$steps) == agg_by_interval$steps,]$interval` contains the maximum number of steps `r agg_by_interval[max(agg_by_interval$steps) == agg_by_interval$steps,]$steps`.
...
1. *Calculate and report the total number of missing values in the dataset (i.e. the total number of rows with `NA`s)*
  ...
  There are *`r sum(is.na(data))`*  missing values in the dataset.
...
3. *Create a new dataset that is equal to the original dataset but with the missing data filled in.*
  ...
  imputings <- merge(imputed_data[!complete.cases(imputed_data),][,-1],agg_by_interval, all.x=T, sort=F,by='interval')
  imputed_data[!complete.cases(imputed_data),] <- imputings[order(imputings$ID),names(imputed_data)]
...
4. *Make a histogram of the total number of steps taken each day and Calculate and report the __mean__ and __median__ total number of steps taken per day. Do these values differ from the estimates from the first part of the assignment? What is the impact of imputing missing data on the estimates of the total daily number of steps?*
  ...
  ```{r ImputedTotalStepsHistogram}
  imputed_agg_by_day <- aggregate(steps~date, imputed_data, sum)
  hist(imputed_agg_by_day$steps, col="green", main="Total Steps Taken by Day (Imputed)", xlab="Daily Total Steps")
...
1. *Create a new factor variable in the dataset with two levels -- "weekday" and "weekend" indicating whether a given date is a weekday or weekend day.*
  ...
  data$day <- factor((weekdays(data$date)=='Saturday' | weekdays(data$date)=='Sunday'),
...
2. *Make a panel plot containing a time series plot (i.e. `type = "l"`) of the 5-minute interval (x-axis) and the average number of steps taken, averaged across all weekday days or weekend days (y-axis).*
  ...
  library(lattice)
  xyplot(steps ~ interval | day, data = agg_by_interval_day, layout = c(1, 2), type = "l")
  ```